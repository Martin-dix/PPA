...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    const mapH = 95;
    doc.addImage(mapImg, "JPEG", 15, y, pageWidth - 30, mapH);
    y += mapH + 15;
    ...
    if (includeHighPoints && state.highPointsLayer && state.highPointsLayer.getLayers().length > 0) {
      ...
      doc.setFontSize(10);
      doc.text("• Top high points are marked on the map above.", 25, y); y += 8;
    }
    ...
    if (includeRelays) {
      if (isRelay) {
        ...
        doc.setFontSize(11);
        doc.text("Dual-leg relay path active. Check critical box for comparison.", 25, y);
        y += 12;
        ...
        state.lastRelaySuggestions.slice(0, 6).forEach((s, i) => {
          ...
          doc.text(`${i+1}. ${grid} — Bottleneck Margin: ${s.bottleneck.toFixed(1)} dB`, 25, y);
          y += 6;
        });
      }
    }
